...
public sealed class CesiumMap : UserControl
{
    ...
    [DefaultValue(true)]
    public bool BaseLayerPicker { get; set; } = true;
    ...
    private void CoreWebView2_WebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        {
            var camera = result.Data.Deserialize<Camera>(serializerOptions)!;
            this.Camera = camera;
    ...
    public async Task<T> ExecuteScriptAsync<T>(string script)
    {
        ...
        void Handler(object sender, CoreWebView2WebMessageReceivedEventArgs args)
        {
            ...
            if (result?.Id == id)
            {
                taskCompletionSource.TrySetResult(result.Data.Deserialize<T>(serializerOptions)!);
